...
export function CustomerMarkDeliveredButton({ orderId }: { orderId: string }) {
  ...
  async function handleClick() {
    ...
    const { data, error: fnError } = await supabase.functions.invoke('customer-actions', {
      body: { type: 'mark_delivered', orderId },
    });
    setLoading(false);
  ...
  return (
    <div>
      ...
      >
        {loading ? 'Confirming...' : "I've Received This — Mark Delivered"}
      </button>
    </div>
  );
        }
